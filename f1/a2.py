...
"""
F1A2 Mininet
use: sudo python3 a2.py --topology [ sl | ft ] --dry-run
"""
...
PAIRS = [
    ...
    {"id": "p2", "client": "h2", "server": "h6", "server_ip": "10.0.2.2", "port": 5202},
    {"id": "p3", "client": "h3", "server": "h7", "server_ip": "10.0.3.1", "port": 5203},
    {"id": "p4", "client": "h4", "server": "h8", "server_ip": "10.0.3.2", "port": 5204},
]
...
def run_single_pair(pair, pkt_size, rep, topology, out_dir, results, errors, dry_run, hosts):
    ...
    if data:
        ...
        errors.append(f"{pair['id']}: fallo en iperf3")
        results[pair["id"]] = None
...
def run_rep(pkt_size, rep, topology, out_dir, dry_run, hosts):
    ...
    errors = []
    threads = [
        threading.Thread(target=run_single_pair,
...
def run_experiment(topology, dry_run):
    ...
    print(f"\n{'='*72}")
    print(f"  F1-A2 (Mininet) | TopologÃ­a: {topology.upper()}")
    ...
    try:
        for pkt_size in PKT_SIZES:
            print(f"-- PKT {pkt_size:>4d} B " + "-"*54)
            throughputs_per_rep = []

            if not dry_run:
                # Lanzar servidores (uno por par)
                for p in PAIRS:
            ...
            print(f"  â {rep} reps | {final_mean:.2f} Mbps | RSD={rsd_str} | {lr_pct:.1f}% LR\n")
        ...
        # Resumen final
        print(f"\n{'='*72}")
        print(f"  RESUMEN F1-A2 -- {topology.upper()} (Mininet)")
