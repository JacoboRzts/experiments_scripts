...
from f2.common import (
    ...
    measure_baseline, run_main_flow, run_probe_flow, run_ping_client, run_extra_flow
)
...
def run_protocol(executors, topology, protocol, pkt_size, out_dir, dry_run):
    ...
    for level in OVERSUB_LEVELS:
        ...
        while rep < REPS_MAX:
            ...
            t_h2 = threading.Thread(target=run_extra_flow, args=(executors, "H2", 5203, pkt_size, protocol, extra_rate, res_h2, err_h2, dry_run, target_ip))
            ...
            t_h5 = threading.Thread(target=run_extra_flow, args=(executors, "H5", 5204, pkt_size, protocol, extra_rate, res_h5, err_h5, dry_run, target_ip))
            ...
            t_h2.join(timeout=DURATION+30)
            t_probe.join(timeout=DURATION+30)
            t_h5.join(timeout=DURATION+30)
            t_lat.join(timeout=DURATION+30)

            for e in err_main + err_h2 + err_probe + err_h5 + err_lat:
                print(f"\n    WARN: {e}", end="")
...
def run_scenario_s2(executors, topology, protocols, dry_run):
    out_dir = OUTPUT_BASE / topology / "fase2_buffering"
    out_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n{'='*65}")
    print(f"  F2-S2  |  {topology.upper()}  |  {datetime.now():%Y-%m-%d %H:%M:%S}")
    print("  Scenario: H1+H2+H4+H5 -> H7 (max cross-leaf congestion)")
    ...
    print(f"\n\n{'='*65}")
    print(f"  SUMMARY F2-S2 — {topology.upper()}")
    print("  Proto   PKT     Baseline  Inflection  Buffer")
    print("  -----   ---     --------  ----------  ------")
